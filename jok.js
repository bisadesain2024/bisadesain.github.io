...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (event) {
        ...
        const nama = form.querySelector('input[placeholder="nama"]');
        const pesan = form.querySelector('input[placeholder="pesan"]');
        const tanya = form.querySelector('input[placeholder="mau tanya apa silahkan"]');

        // Periksa apakah semua elemen ditemukan
        if (nama && pesan && tanya) {
            const whatsappNumber = '62895338499819'; // Ganti dengan nomor WhatsApp tujuan Anda
            const whatsappURL = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Nama: ${nama.value}\nPesan: ${pesan.value}\nPertanyaan: ${tanya.value}`)}`;
            
            console.log(whatsappURL); // Tambahkan ini untuk debug
            
            window.open(whatsappURL, '_blank');
        } else {
            console.error("Tidak semua input ditemukan. Periksa kembali elemen HTML Anda.");
        }
    });
